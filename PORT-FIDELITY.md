...
ASCII, a `correlation_id` that came off a decoded frame, or a field name or string value off a
decoded frame, and decode validates, so the only way to reach this is to construct a `WakeMessage`
in C++ with invalid UTF-8 and encode it.
